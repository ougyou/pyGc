...
class UpgradeInfo:
    ...
    def matched_level(self):
        ...
        for rule in upgrade_rules:
            if Decimal(self.amount) >= rule.amount or Decimal(self.nights) >= rule.nights or Decimal(
                    self.times_in) >= rule.times_in:
    ...
    def __str__(self):
        return 'card:{} \t amount:{} \t nights:{} \t timesIn:{} \t level:{}'.format(self.card_id, self.amount,
                                                                                    self.nights, self.times_in,
                                                                                    self.level)
